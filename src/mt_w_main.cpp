...
int main(int argc, char *argv[])
{
  ...
  const int desiredUpdateRate = 120;
  ...
  try {
    ...
    RCLCPP_INFO(node->get_logger(), "Found a device with ID: %s @ port: %s, baudrate: %d",
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
      "Setting radio channel to " << desiredRadioChannel << " and enabling radio...");
    ...
    do{
      ...
      while (true) {
        ...
        if (nextCount != connectedMTWCount) {
          RCLCPP_INFO_STREAM(node->get_logger(),
            "Number of connected MTWs: " << (int)nextCount << ". Press 'y' to start measurement or 'q' to end node.");
    ...
    XsOutputSettings desiredSettings = XOS_Timestamp_PacketCounter & XOS_OrientationMode_Matrix &
     ...
     * ros topic hz shows around 120 Hz for each MTw. Echoing
     * 'free_acc' topic has less latency with higher loop_rate values, though.
     */
